...
export function latexToText(input: string): string {
  ...
  s = s.replace(
    /\\begin\s*\{(?:pmatrix|bmatrix|vmatrix|matrix|align|aligned|equation|gather)\*?\}/g,
    "",
  );
  s = s.replace(
    /\\end\s*\{(?:pmatrix|bmatrix|vmatrix|matrix|align|aligned|equation|gather)\*?\}/g,
    "",
  );
  s = s.replace(/\\\\/g, "\n");
  s = s.replace(/&/g, "  ");
  ...
  s = s.replace(
    /\\(text|mathrm|operatorname|mathbf|mathit|overline|bar|vec|hat|tilde)\s*\{([^{}]*)\}/g,
    "$2",
  );
  ...
  for (let iter = 0; iter < 3; iter++) {
    ...
    s = s.replace(
      /\\(frac|dfrac|tfrac)\s*\{\s*([^{}]*?)\s*\}\s*\{\s*([^{}]*?)\s*\}/g,
      (_, _cmd, a, b) => `(${a.trim()})/(${b.trim()})`,
    );
  }
