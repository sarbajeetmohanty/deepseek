...
function Dashboard() {
  ...
  const [pdfStatus, setPdfStatus] = useState<
    "idle" | "working_phase1" | "working_phase2" | "done" | "error"
  >("idle");
  ...
  const savePromptMut = useMutation({
    mutationFn: () =>
      saveUserPrompt({ data: { name: newPromptName.trim(), text: newPromptText.trim() } }),
    onSuccess: (newPrompt) => {
  ...
  async function handlePdfUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      let queueIndex = 0;
      const phase1Pages: { pageNumber: number; text: string }[] = [];
      ...
      const worker = async () => {
        while (queueIndex < images.length) {
          ...
          try {
            ...
            phase1Pages.push({
              pageNumber: pageNum,
              text: `[Page ${pageNum}: OCR text could not be extracted]`,
            });
          } finally {
      ...
      const workers = Array.from({ length: Math.min(MAX_CONCURRENCY, images.length) }, () =>
        worker(),
      );
...
4. If you are generating new questions based on the text, the facts, options, and answers MUST be 100% logically sound and strictly derived from the provided context.`;
        ...
        try {
          ...
          console.warn(
            "Phase 2 prompt formatting failed, falling back to raw OCR text:",
            phase2Err,
          );
          toast.warning(
            "Custom prompt formatting encountered a filter; loaded raw OCR text instead.",
          );
      ...
      console.error(cause);
      setPdfError(
        cause instanceof Error ? cause.message : "Something went wrong while extracting.",
      );
      setPdfStatus("error");
    }
  }
  ...
  const create = useMutation({
    mutationFn: (variables?: { title?: string; rawText?: string }) =>
      createBatch({
        data: {
            ...
            <div className="space-y-3 col-span-full border-t border-border/50 pt-4 mt-2">
                ...
                {prompts.map((p: any) => (
                  <div key={p.id} className="relative group flex items-center">
                    ...
                    >
                      ...
                      >
                        <path
                          d="M5.5 1C5.22386 1 5 1.22386 5 1.5C5 1.77614 5.22386 2 5.5 2H9.5C9.77614 2 10 1.77614 10 1.5C10 1.22386 9.77614 1 9.5 1H5.5ZM3 3.5C3 3.22386 3.22386 3 3.5 3H11.5C11.7761 3 12 3.22386 12 3.5C12 3.77614 11.7761 4 11.5 4H11V12C11 12.5523 10.5523 13 10 13H5C4.44772 13 4 12.5523 4 12V4H3.5C3.22386 4 3 3.77614 3 3.5ZM5 4V12H10V4H5Z"
                          fill="currentColor"
                          fillRule="evenodd"
                          clipRule="evenodd"
                        ></path>
              ...
              {(isAddingPrompt || isEditingPrompt) && (
                <div className="rounded-md border border-border p-4 space-y-3 bg-secondary/10 mt-2">
                  <Input
                    placeholder="Prompt Name (e.g. History Translation)"
                    value={newPromptName}
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      type="button"
                      size="sm"
                      disabled={savePromptMut.isPending || editPromptMut.isPending}
                      onClick={() => {
                        if (!newPromptName.trim() || !newPromptText.trim()) return;
                        if (isEditingPrompt && editingPromptId) {
                          editPromptMut.mutate({
                            id: editingPromptId,
                    ...
                    >
                      {savePromptMut.isPending || editPromptMut.isPending
                        ? "Saving..."
                        : isEditingPrompt
                          ? "Update Prompt"
                          : "Save Prompt"}
            ...
            <div className="space-y-3 col-span-full mb-4">
                ...
                <div className="flex flex-col items-center gap-3 pointer-events-none">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      We'll automatically extract the text using the selected prompt
                    </p>
                  </div>
                </div>
              </div>
              {pdfStatus.startsWith("working") && (
                <div className="mt-2 text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 flex-1 bg-secondary rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(pdfProgress.done / Math.max(1, pdfProgress.total)) * 100}%`,
                          }}
                        />
                      </div>
                  ...
                  {pdfStatus === "working_phase2" && (
                    <p className="mt-1 text-muted-foreground">Applying global context prompt...</p>
                  )}
                </div>
              )}
